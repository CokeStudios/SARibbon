...
- 新增：Ribbon 按钮自动继承其关联动作的自动化标识（objectName/accessibleName），方便 Squish、TestComplete 等自动化测试工具定位按钮（用户显式指定的标识不会被覆盖）（Closes #121）；
- 新增：Gallery 拉伸系数 `setStretchFactor`，同一面板放置多个 Gallery 时可按权重分配水平空间（默认不设置时保持原有的均分行为不变）（Closes #47）；
- 新增：标题栏图标系统菜单的"移动"与"大小"功能，点击后可用方向键移动/缩放窗口（与 Windows 原生系统菜单一致；最大化状态下会先还原；非 Windows 平台这两项明确置灰）（关联 #130）；
- 新增：主窗口边框绘制开关与颜色属性，无边框窗口落在同色背景上时可开启 1px 边框区分边界（默认关闭不影响既有外观，颜色可跟随主题）（Closes #154）；
- 新增：无边框模式（Windows）下拖动标题栏可触发系统贴边行为——拖到屏幕左/右边缘半屏、拖到顶部最大化、拖离还原，与 Office/浏览器一致；标题栏上的按钮、页签等控件不受影响（Closes #31）；
- 新增：无边框窗口的系统阴影开关（Windows），开启后获得 DWM 系统阴影；最大化时系统不绘制阴影属正常行为（Closes #129）；
- 新增：快速访问工具栏的自定义支持——在"接口自定义"对话框中可把动作加入快速访问栏、移除、调整顺序，并可与 Ribbon 定制一起保存/加载（Closes #67，Gitee IA62AX）；
- 文档：补充"在 Gallery 与面板中放置自定义控件"的说明与可运行示例（复选框、下拉框等控件经面板小项或 Gallery 弹出视口放置，含所有权与尺寸约束说明）（Closes #78）；
- 确认 Gitee IBEIH9：按钮群与快速访问栏的动作插入顺序（追加、在任意位置插入、移除）已由回归测试固化，当前版本原生支持 `insertAction(before, ...)`，无需额外接口；
- 文档：补充按钮群与快速访问栏的动作顺序调整用法（插入/移除/移动用 Qt 原生接口组合即可，含自定义控件时的状态保留说明）（关联 #60）；
- 修复：非当前显示页签（category）里的大按钮宽度异常收缩的问题（2.9.4 引入的回归）。
  大按钮最小宽度的计算此前改为依赖面板的实时高度，而隐藏页签的面板在首次被访问前
  尚未获得真实几何（其尺寸提示被 Qt 布局系统提前查询并缓存），导致最小宽度退化为
